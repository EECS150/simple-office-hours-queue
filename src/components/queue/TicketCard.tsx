...
import {
  BUTTONS_DISABLED_WAIT_MSG,
  BUTTONS_DISABLED_WAIT_TIME,
} from "../ticket-page/TicketButtons";
...
const TicketCard = (props: TicketCardProps) => {
  ...
  const canUserClickOnTicket =
    ticket.isPublic || isStaff || isIntern || ticket.createdByUserId === userId;
  ...
  const handleHelpTicket = async () => {
    onClickWrapper(async () => {
      const res = await assignTicketsMutation.mutateAsync({
        ticketIds: [ticket.id],
      });
      if (res.length > 0) {
  ...
  const handleCloseTicket = async () => {
    await onClickWrapper(() =>
      closeTicketMutation.mutateAsync({ ticketId: ticket.id }),
    )();
  };
  ...
  return (
    <Box
      ...
      _hover={
        canUserClickOnTicket
          ? { backgroundColor: hoverColor, transition: "0.3s" }
          : {}
      }
    >
      <Flex direction="row" mt={-7} ml={-6}>
        <Text fontStyle="italic" color="gray.500">
          {idx > -1
            ? `#${idx} (${uppercaseFirstLetter(ticket.status)})`
        ...
        <Flex hidden={!ticket.isPublic}>
          ...
          {usersInGroup === undefined ? (
            ...
            <Text>
              Public ({usersInGroup.length} student
              {usersInGroup.length === 1 ? "" : "s"} in group)
            </Text>
          )}
        </Flex>
      </Flex>
      ...
      <Flex justifyContent="space-between">
        ...
        <Flex flexDirection="column">
          <Text
            ...
            alignSelf="flex-end"
          >
            Created {timeDifferenceInMinutes(new Date(), ticket.createdAt)}{" "}
            minute(s) ago
          </Text>
          <Text
            hidden={ticket.status !== TicketStatus.ASSIGNED}
            fontSize="lg"
            mb={2}
          >
          ...
          <Box textAlign="right">
            ...
            <Button
              ...
              colorScheme="red"
            >
              {ticket.status === TicketStatus.ABSENT ? "Unmark" : "Mark"} as
              absent
            </Button>
            <Button
              title={areButtonsDisabled ? BUTTONS_DISABLED_WAIT_MSG : ""}
            ...
            {usersInGroup === undefined && ticket.isPublic ? (
              ...
              <Button
                ...
                isLoading={areButtonsLoading}
                onClick={
                  isCurrentUserInGroup ? handleLeaveGroup : handleJoinGroup
                }
